...
public class SubsequencePattern {
 ...
 public int findSPMCountMemo(String str, String pat) {
  Integer[][] dp = new Integer[str.length()][pat.length()];
  return findSPMCountRecursiveMemo(str, pat, 0, 0, dp);
 }

 private int findSPMCountRecursiveMemo(String str, String pat, int strIndex, int patIndex, Integer[][] dp) {
  ...
  if (dp[strIndex][patIndex] == null) {
   int c1 = 0;
   if (str.charAt(strIndex) == pat.charAt(patIndex)) {
    c1 = findSPMCountRecursiveMemo(str, pat, strIndex + 1, patIndex + 1, dp);
   }

   int c2 = findSPMCountRecursiveMemo(str, pat, strIndex + 1, patIndex, dp);
 ...
 public static void main(String[] args) {
  SubsequencePattern spm = new SubsequencePattern();
  System.out.println(spm.findSPMCount("baxmx", "ax"));
  System.out.println(spm.findSPMCount("tomorrow", "tor"));
  System.out.println(spm.findSPMCountMemo("baxmx", "ax"));
  System.out.println(spm.findSPMCountMemo("tomorrow", "tor"));  
  System.out.println(spm.findSPMCountDp("baxmx", "ax"));
  System.out.println(spm.findSPMCountDp("tomorrow", "tor"));  
 }
}