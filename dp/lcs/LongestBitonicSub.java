...
class LongestBitonicSub {
  ...
  private int findLBSLengthMemo(int[] nums) {
    ...
    int maxLength = 0;
    for(int i=0; i<nums.length; i++) {
      int c1 = findLDSLengthMemo(lds, nums, i, -1);
      int c2 = findLDSLengthReverseMemo(ldsRev, nums, i, -1);
      maxLength = Math.max(maxLength, c1+c2-1);
    }
  ...
  // find the longest decreasing subsequence from currentIndex till the end of the array
  private int findLDSLengthMemo(Integer[][] dp, int[] nums, int currentIndex, int previousIndex) {
    if(currentIndex == nums.length)
      return 0;
    ...
    if(dp[currentIndex][previousIndex+1] == null) {
      ...
      int c1 = 0;
      if(previousIndex == -1 || nums[currentIndex] < nums[previousIndex])
        c1 = 1 + findLDSLengthMemo(dp, nums, currentIndex+1, currentIndex);

      // excluding the number at currentIndex
      int c2 = findLDSLengthMemo(dp, nums, currentIndex+1, previousIndex);
  ...
  // find the longest decreasing subsequence from currentIndex till the beginning of the array
  private int findLDSLengthReverseMemo(Integer[][] dp, int[] nums, int currentIndex, int previousIndex) {
    if(currentIndex < 0)
      return 0;
    ...
    if(dp[currentIndex][previousIndex+1] == null) {
      ...
      int c1 = 0;
      if(previousIndex == -1 || nums[currentIndex] < nums[previousIndex])
        c1 = 1 + findLDSLengthReverseMemo(dp, nums, currentIndex-1, currentIndex);

      // excluding the number at currentIndex
      int c2 = findLDSLengthReverseMemo(dp, nums, currentIndex-1, previousIndex);
  ...
  private int findLBSLengthDp(int[] nums) {
  ...
  public static void main(String[] args) {
    LongestBitonicSub lbs = new LongestBitonicSub();
    int[] nums = {4,2,3,6,10,1,12};
    System.out.println(lbs.findLBSLength(nums));
    nums = new int[]{4,2,5,9,7,6,10,3,1};
    System.out.println(lbs.findLBSLength(nums));
    System.out.println(lbs.findLBSLengthMemo(nums));    
     System.out.println(lbs.findLBSLengthDp(nums));      
  }
}
